...
class Trainer:
    ...
    def train(self):
        ...
        step = 0
        def run_epoch(split):
            print(f'run_epoch: {split}')
            nonlocal step
            is_train = split == 'train'
            model.train(is_train)
            data = self.train_dataset if is_train else self.test_dataset
            loader = DataLoader(data, batch_size=config.batch_size, num_workers=config.num_workers, shuffle=is_train,prefetch_factor=2)
        ...
        for epoch in range(config.max_epochs):
            ...
            run_epoch('train')
            # print(f'self.test_dataset: {self.test_dataset}')
