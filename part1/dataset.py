...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        document = self.data[idx] # get the document at the index
        truncated_length = random.randint(4, int(self.block_size*7/8)) # random truncation length
        truncated_document = document[:truncated_length] # truncate the document
        
        prefix_length = random.randint(0, truncated_length)
        masked_content_length = int(truncated_length/4) + random.randint(-int(truncated_length/8), int(truncated_length/8))

        suffix_length = truncated_length - prefix_length - masked_content_length
        ...
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR*(self.block_size - len(prefix + suffix + masked_content) -len(self.MASK_CHAR) * 2 + 1)
